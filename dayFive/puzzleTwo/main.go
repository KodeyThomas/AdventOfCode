...
func main() {
	...
	fmt.Printf("Flattening Tensor into Matrix took %s\n", flatteningTensorTime)
	fmt.Print("\n")
	fmt.Printf("Program Execution took %s\n", elapsed)
	fmt.Print("\n")

	var defintion int
	fmt.Print("Enter Matrix Image Definition: ")
	fmt.Scanln(&defintion)
	printMatrix(hydrothermalMatrix, maxX, maxY, defintion)
}
